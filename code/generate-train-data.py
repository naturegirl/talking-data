...
def read_agegroups():
    d = {}
    with open("../processed_data/agegroups.txt", "r") as f:
        for line in f:
            idx, group = line.rstrip().split(":")
            d[group] = int(idx)
    return d
...
df["group_id"] = df["group"].map(lambda s: agegroups_dict[s])
